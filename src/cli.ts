...
program
  .name('codereview')
  .description('Local-first AI code review agent. No cloud, no API keys, no subscriptions.')
  .version('1.1.0');
...
program
  ...
  .option('--export-output <path>', 'File path for export output (stdout if omitted)')
  .action(async (opts) => {
    await reviewCommand({
      base: opts.base,
      staged: opts.staged,
  ...
  .action((opts) => {
    if (opts.latest) {
      ...
      for (const issue of entry.issues) {
        ...
        console.log(`  [${issue.severity.toUpperCase()}]${loc} ${issue.description}`);
      }
      return;
    }
    ...
    for (const e of entries) {
      ...
      console.log(`  ${ts}  [${e.model}]  ${e.issueCount} issue(s)  (${e.mode})`);
    }
    console.log('');
  });
